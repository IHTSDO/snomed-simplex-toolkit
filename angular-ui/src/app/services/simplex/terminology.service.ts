...
export class TerminologyService {
  ...
  private handleError<T>(operation = 'operation', result?: T) {
    return (error: any): Observable<T> => {
      ...
      console.error(error);
      this._snackBar.open(`Error in terminology service request: ${error.message}`, 'Dismiss', {
        duration: 5000
      });
  ...
  /**
   * Retrieves refsets from the native Snowstorm API using the branch path
   * @param branchPath The branch path (e.g., 'MAIN/SNOMEDCT-ES/SNOMEDCT-AR')
   * @param moduleId The module ID to filter refsets
   * @param offset Optional offset for pagination (default: 0)
   * @param limit Optional limit for pagination (default: 50)
   * @returns Observable with the refsets array
   */
  getRefsetsFromNativeApi(branchPath: string, moduleId: string, offset: number = 0, limit: number = 50): Observable<any> {
    ...
    console.log('getRefsetsFromNativeApi', branchPath, moduleId, offset, limit, baseEcl);

    // Encode the branch path and ECL for URL
    const encodedBranchPath = encodeURIComponent(branchPath);
    const encodedEcl = encodeURIComponent(baseEcl);
    
    // Construct the native API URL - use branchPath directly according to API documentation
    const requestUrl = `/snowstorm/snomed-ct/${encodedBranchPath}/concepts?ecl=${encodedEcl}&includeLeafFlag=false&form=inferred&offset=${offset}&limit=${limit}`;
    ...
    return this.http.get<any>(requestUrl, { headers })
      .pipe(
        catchError(this.handleError<any>('getRefsetsFromNativeApi', {}))
      );
  }
  ...
  getDerivativesFromServer(): Observable<any> {
    ...
    const excludedConceptIds = [
      '446608001', // SNOMED CT to ICD-O simple map reference set
      '447562003', // SNOMED CT to ICD-10 extended map reference set
      '711112009', // ICNP diagnoses to SNOMED CT simple map
      '712505008', // ICNP interventions to SNOMED CT simple map
    ...
    return this.http.get<any>(requestUrl, { headers })
      .pipe(
        map((response: any) => {
          // Filter out excluded concept IDs from referenceSets
          if (response?.referenceSets) {
            const filteredReferenceSets: any = {};
            Object.keys(response.referenceSets).forEach(key => {
              if (!excludedConceptIds.includes(key)) {
                filteredReferenceSets[key] = response.referenceSets[key];
              }
            });
            response.referenceSets = filteredReferenceSets;
          }
          
          // Filter out excluded concept IDs from memberCountsByReferenceSet
          if (response?.memberCountsByReferenceSet) {
            const filteredMemberCounts: any = {};
            Object.keys(response.memberCountsByReferenceSet).forEach(key => {
              if (!excludedConceptIds.includes(key)) {
                filteredMemberCounts[key] = response.memberCountsByReferenceSet[key];
              }
            });
            response.memberCountsByReferenceSet = filteredMemberCounts;
          }
