...
export class EclBuilderService {
  ...
  setBranchPath(branchPath: string): void {
    this.branchPath = branchPath || 'MAIN';
  }
  ...
  stringToModel(eclString: string): Observable<EclExpressionConstraint> {
    ...
    return this.http
      .post<any>(`${this.getApiBaseUrl()}/string-to-model`, normalized, options)
  ...
  modelToString(model: EclExpressionConstraint): Observable<string> {
    ...
    return this.http
      .post<any>(`${this.getApiBaseUrl()}/model-to-string`, clonedModel, { headers })
  ...
  getDomainAttributes(parentConceptId: string): Observable<EclDomainAttribute[]> {
    ...
    const requestUrl =
      `${this.getApiBaseUrl()}/domain-attributes` +
      `?branchPath=${encodeURIComponent(this.branchPath)}&parentIds=${encodeURIComponent(parentConceptId)}`;
    ...
    const walk = (target: any): any => {
      ...
      Object.keys(target).forEach((prop) => {
        ...
        if (prop === 'wildcard' && target[prop] === true) {
          ...
          if (target.conceptId && !`${target.conceptId}`.includes('|')) {
            target.conceptId = `${target.conceptId} |${target.term}|`;
          }
          delete target.term;
          return;
        } else if (prop === 'returnAllMemberFields' || prop === 'altIdentifier') {
          delete target[prop];
          return;
        }
  ...
  private transformOut(model: any): void {
    ...
    Object.keys(model).forEach((prop) => {
      ...
      if (prop === 'altIdentifier' || prop === 'returnAllMemberFields') {
        delete model[prop];
        return;
      }
