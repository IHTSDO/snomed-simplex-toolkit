...
export class EclBuilderComponent implements OnInit, OnChanges, OnDestroy {
  ...
  @Output() unsupportedFeaturesChange = new EventEmitter<string[]>();
  ...
  onModelChanged(): void {
    ...
    this.unsupportedFeaturesChange.emit(unsupported);
    if (unsupported.length) {
      this.unsupportedMessage = `Unsupported in this visual builder: ${unsupported.join(', ')}.`;
      this.updateValidity(false);
      return;
    }
